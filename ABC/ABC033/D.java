...
public class D {
	...
	static <T extends Comparable<? super T>> int upperBound(ArrayList<T> a, T k) {
		int n = a.size();
		int l = -1, r = n;
		while (r - l > 1) {
			int m = (l + r)/2;
	...
	static void solve(){
		...
		for (int i=0; i<n; i++) {
			...
			for (int j=0; j<n; j++) {
				if (i == j) continue;
				a.add(Math.atan2(ps[j].getY() - ps[i].getY(), ps[j].getX() - ps[i].getX()));
				a.add(a.get(a.size()-1) + Math.PI*2);
			}

			Collections.sort(a);
			dump(a);
			for (int j=0; j<n-1; j++) {
				y += upperBound(a, a.get(j) + Math.PI/2 + 1e-10) - lowerBound(a, a.get(j) + Math.PI/2 - 1e-10);
				z += lowerBound(a, a.get(j) + Math.PI) - upperBound(a, a.get(j) + Math.PI/2 + 1e-10);
			}
		}
